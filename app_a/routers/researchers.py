...
async def search_researchers(
    ...
    ## パラメータ
    - **query**: 全文検索クエリ（業績テキストを検索）
    - **org**: 機関フィルター（カンマ区切りでOR条件）
      - 例: `org=歴博` → org1またはorg2が「歴博」の研究者
      - 例: `org=歴博,国文研` → 「歴博」または「国文研」の研究者
    - **initial**: 英語名のイニシャル（A-Z、一文字）
    - **page**: ページ番号
    - **page_size**: 1ページあたりの件数（最大100）

    ## レスポンス
    - **total**: 検索条件に一致する研究者の総数
    - **researchers**: 研究者リスト
      - **snippets**: クエリにマッチした業績（最大5件）
        - queryパラメータがある場合のみ返却
        - 各スニペットにはresearchmapへのURLが含まれる
        - マッチ箇所は`<mark>`タグでハイライト

    ## サンプルクエリ
    | クエリ | 件数 | 説明 |
    |--------|------|------|
    | (なし) | 224 | 全件取得 |
    | `query=情報` | 143 | 全研究所にヒット |
    | `query=ジェンダー` | 22 | 全研究所にヒット |
    | `query=インドネシア` | 22 | 機構本部の研究者も含む |
    | `org=地球研` | 22 | 機関フィルター |
    | `initial=K` | 31 | イニシャルフィルター |
    | `query=ジェンダー&initial=A` | 1 | 複合条件 |
    | `query=室町` | 12 | 地球研・民博にはいない |
    | `query=誓詞帳` | 1 | 未知語の検索 |
    | `query=ローレンシウム` | 0 | 該当なし |
    """
